...
class Command {
     ...
     * walk paper n lines
     * 
     * @param  {Number} 0-255 Lines of paper
     * @return {Command}
     */
    newLine (num = 1) {
        for (let i = 0; i < num; i++) {
    ...
    fontSize (size = 0) {
        ...
        if (size > 7) throw "size must not be greater than 8!";
    ...
    barcode (code) {
        code = String(code);
        if (code.length !== 12 || !/^[0-9]+$/.test(code)) throw "The code length is fixed at 12 bits, and can only be numbers!";
        this._queue.push(...this._cmd["ESA"], 1);
